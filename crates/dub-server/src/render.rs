...
fn build_dub(
    ...
    for &(fi, s) in segs.iter() {
        ...
        if need_synth {
            ...
            let tok_cap: u32 = (((expected_dur * 75.0 * 1.5).ceil() as u32) + 32).clamp(64, 768);
        ...
        if multitake_on && need_synth && !kept_original && !cloud_tts_on && engine.is_some() {
            ...
            if take1_ok {
                ...
                let tok_cap: u32 = ((((s.end - s.start).max(0.6) * 75.0 * 1.5).ceil() as u32) + 32).clamp(64, 768);
    ...
    if run_qc_asr && !qc_list.is_empty() {
        ...
        if !bad_idx.is_empty() {
            ...
            for &i in &bad_idx {
                ...
                let cap: u32 = (((e_dur * 75.0 * 1.5).ceil() as u32) + 32).clamp(64, 768);
