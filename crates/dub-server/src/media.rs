...
/// Вырезать фрагмент [start, end] с 5 мс краевыми микро-фейдами в mono @ sr Гц.
/// Исключает резкие щелчки на границах референсного аудиоклипа.
pub fn trim_faded(src: &Path, dst: &Path, start: f64, end: f64, sr: u32) -> Result<(), String> {
    let dur = (end - start).max(0.05);
    let ss = format!("{:.3}", start);
    let to = format!("{:.3}", end);
    let ar = sr.to_string();
    let fade_d = (dur * 0.05).min(0.010).max(0.003); // 3-10 мс
    let fade_st = (dur - fade_d).max(0.0);
    let af = format!("afade=t=in:st=0:d={fade_d:.3},afade=t=out:st={fade_st:.3}:d={fade_d:.3}");
    ...
    if run_ff(&[
        ...
        OsStr::new("-ar"), OsStr::new(&ar),
        dst.as_os_str(),
    ]).is_err() {
        // Fallback на обычный trim без afade
        trim(src, dst, start, end, sr)?;
    }
    Ok(())
}

/// Очистить референс клонирования голоса (FFT Spectral Denoise + Band-Pass 75Hz..7500Hz).
/// Удаляет фоновый стационарный шум, комнатный гул и фазовые артефакты сепарации перед отправкой в Higgs TTS.
/// В случае сбоя afftdn делает прозрачный fallback на базовый highpass/lowpass фильтр.
pub fn denoise_reference(src: &Path, dst: &Path) -> Result<(), String> {
    let is_same = src == dst;
    let target_dst = if is_same {
        dst.with_extension("tmp_denoise_in_place.wav")
    } else {
        dst.to_path_buf()
    };

    let af = "highpass=f=75,lowpass=f=7500,afftdn=nf=-42:nt=w";
    let res = run_ff(&[
        OsStr::new("-y"), OsStr::new("-i"), src.as_os_str(),
        OsStr::new("-af"), OsStr::new(af),
        OsStr::new("-ac"), OsStr::new("1"),
        OsStr::new("-ar"), OsStr::new("16000"),
        OsStr::new("-c:a"), OsStr::new("pcm_s16le"),
        target_dst.as_os_str(),
    ]);

    if res.is_err() {
        // Fallback: базовый полосовой фильтр без FFT-денойза
        let af_fb = "highpass=f=75,lowpass=f=7500";
        run_ff(&[
            OsStr::new("-y"), OsStr::new("-i"), src.as_os_str(),
            OsStr::new("-af"), OsStr::new(af_fb),
            OsStr::new("-ac"), OsStr::new("1"),
            OsStr::new("-ar"), OsStr::new("16000"),
            OsStr::new("-c:a"), OsStr::new("pcm_s16le"),
            target_dst.as_os_str(),
    ...
    if is_same && target_dst.is_file() {
        let _ = std::fs::rename(&target_dst, dst);
    }
    Ok(())
}
