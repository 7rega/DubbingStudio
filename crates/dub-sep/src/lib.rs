...
fn separate_windowed(
    ...
    let vocals = out_dir.join("vocals.wav");
    let instrumental = out_dir.join("instrumental.wav");
    let f = |secs: f64| -> u64 { (secs * sr as f64).round() as u64 }; // сек -> фреймы
    ...
    for i in 0..n_win {
        ...
        let chunk = wav::read_f32_range(mix_wav, g0 as u32, (g1 - g0) as u32).map_err(SepError::Wav)?;
        ...
        let to_local = |gframe: u64| -> usize { (gframe.saturating_sub(g0)) as usize * ch as usize };
        ...
        if i + 1 < n_win {
            ...
            let tail_start = to_local(next_cut).saturating_sub(cf * ch as usize / 2).max(body_start_local).min(s.len());
            out_voc.write(&s[body_start_local.min(s.len())..tail_start]).map_err(SepError::Wav)?;
