...
pub struct Audio {
    ...
    #[serde(default)]
    pub gain_db: f64,
    /// Гейн сгенерированной дорожки речи (TTS/дубляж) перед сведением, dB (дефолт 0 = без изменений).
    #[serde(default)]
    pub voice_gain_db: f64,
    /// Гейн фоновой музыки/инструментала (BGM) перед сведением, dB (дефолт 0 = без изменений).
    #[serde(default)]
